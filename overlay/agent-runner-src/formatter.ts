...
// Why: SDK auto-compaction summarizes the conversation as free-form prose,
// dropping the per-message `from="..."` attributes. Post-compact, the agent
// would lose track of which channel it was in and default to whichever
// destination its persona named most prominently (e.g. #main). This tag is
// emitted at the top of every prompt so even a freshly-summarized session
// has a load-bearing routing anchor in immediate context.
function formatRoutingTag(messages: MessageInRow[]): string {
  const first = messages[0];
  if (!first?.channel_type || !first?.platform_id) return '';
  const dest = findByRouting(first.channel_type, first.platform_id);
  if (!dest) return '';
  return `\n<routing reply_to_channel="${escapeXml(dest.name)}" />`;
}
...
export function formatMessages(messages: MessageInRow[]): string {
  const header = `<context timezone="${escapeXml(TIMEZONE)}" />${formatRoutingTag(messages)}\n`;
