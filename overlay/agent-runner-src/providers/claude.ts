    ...
    async function* translateEvents(): AsyncGenerator<ProviderEvent> {
      ...
      for await (const message of sdkResult) {
        ...
        if (message.type === 'system' && message.subtype === 'init') {
          ...
          // Compaction is internal SDK plumbing, not a turn result. Emitting it
          // as 'result' caused the host to post "Context compacted (...)" into
          // the user's channel. Surface it as 'progress' so it lands in logs
          // for debugging but never reaches Discord/etc.
