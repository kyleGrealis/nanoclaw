/**
 * Gemini provider v2 — built on `@google/genai` (the supported SDK).
 *
 * Why a v2:
 *   - The original `gemini.ts` uses `@google/generative-ai`, which Google has
 *     marked deprecated. The legacy `Tool` shape is a discriminated union
 *     (`FunctionDeclarationsTool | CodeExecutionTool | GoogleSearchRetrievalTool`)
 *     so a single tools array element is one variant — and the only search
 *     variant is `googleSearchRetrieval`, which is a Gemini 1.5-era thing.
 *   - On `@google/genai`, the unified `Tool` interface lets you put
 *     `functionDeclarations`, `googleSearch`, `codeExecution`, etc. on the
 *     SAME object. That's how Gemini 2.5 mixes its built-in `googleSearch`
 *     grounding with custom function calling in one request.
 *
 * Drop-ins from v1 we deliberately preserve:
 *   - Persona file load (CLAUDE.md + CLAUDE.local.md)
 ...
 *     prefix when a server name is a substring of the tool name; observed on
 *     `brave_search__brave_web_search` arriving as `brave_web_search`).
 ...
 *
 * Net new in v2:
 *   - `googleSearch: {}` ships on the same Tool object as `functionDeclarations`.
 *     This replaces the Brave Search MCP server for grounded web lookups while
 *     leaving every other MCP tool intact.
 *
 * Registers as the `gemini-v2` provider name. The legacy `gemini` provider
 * stays in place as a one-flag rollback (`provider: "gemini"` in container.json).
 */
import {
  GoogleGenAI,
  type Chat,
...
function log(msg: string): void {
  console.error(`[gemini-v2-provider] ${msg}`);
}
...
export class GeminiV2Provider implements AgentProvider {
  ...
  private async ensureMcpClients(): Promise<void> {
    for (const [name, config] of Object.entries(this.mcpConfigs)) {
      ...
      const client = new Client(
        { name: "nanoclaw-gemini-v2-provider", version: "1.0.0" },
        { capabilities: { tools: {} } },
      );
...
registerProvider('gemini-v2', (opts) => new GeminiV2Provider(opts));
