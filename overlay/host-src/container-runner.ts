...
async function spawnContainer(session: Session): Promise<void> {
  ...
  // Log stderr at INFO so post-mortems on hangs and silent failures actually
  // have something to read. Was DEBUG to avoid spam, but the spam is bounded
  // (poll heartbeats every 30s + a few lines per turn) and the diagnostic
  // value of seeing what agent-runner said when a container froze far
  // outweighs the noise.
