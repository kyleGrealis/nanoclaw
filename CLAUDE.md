...
11. ~~**`container/agent-runner/src/mcp-tools/bash.ts`** + **`bash.instructions.md`** — bash MCP + Host OPSEC deny-list.~~ **RETIRED 2026-05-04** when Andy reverted from Gemini to Claude. The Claude SDK ships a built-in Bash tool, making this overlay redundant. Files trashed from both install and operator overlay; if a future provider lacks Bash, the `add-gemini-provider`-style skill can re-add equivalent.
12. ~~**`container/agent-runner/src/providers/gemini.ts`** + **`src/providers/gemini.ts`** + **`@google/genai` dep** — Gemini provider on `@google/genai`.~~ **RETIRED 2026-05-04**. Andy's Gemini experiment ran 2026-04-30 → 2026-05-04 and was reverted after persistent fabrication and unsolicited-action behavior in #devops. Provider files trashed; dep removed via `bun remove @google/genai`. Re-add path documented in `.claude/skills/add-gemini-provider/SKILL.md` if a future Gemini run is wanted.
...
14. **Dispatch system — orchestrator → ephemeral worker, end-to-end wired 2026-05-02.** Andy only. Six pieces:
    ...
    - **`dispatch-scopes/<scope>/`** — four scope templates, each with `CLAUDE.md` + `container.json`. All four flipped to **Claude Sonnet 4.6** on 2026-05-04 (was Gemini 3 Pro/Flash). `research` (RO `/home/kyle` + WebSearch + WebFetch + Bash), `devops` (RW `/home/kyle` + ssh keys + Bash), `data` (RO `/home/kyle` + RW `/mnt/piCloud` + Bash), `plain` (RO `/home/kyle` + Bash only). Worker personas explicitly forbid `send_message`, `schedule_task`, `dispatch_task`, etc. — workers have no chat channel, no recursion, no memory.
...
15. **`container/agent-runner/src/mcp-tools/core.ts`** — Andy only for now (Milton not yet patched). Copy from `overlay/agent-runner-src/mcp-tools/core.ts`. Strips `<internal>...</internal>` from the `text` argument inside `send_message`, `send_file`, and `edit_message` before it hits `messages_out`. Originally added because of Gemini's habit of wrapping prelude reasoning in `<internal>` tags *inside* tool arguments (which bypasses `formatter.ts:stripInternalTags` — that only runs on free-text output). Observed in #misc on 2026-05-02 (USATF rulebook reply led with the entire `<internal>` block). The persona at `groups/andy/CLAUDE.local.md` promises `<internal>` is "logged but not sent" — this overlay item is what makes that promise true on tool-arg text. **Kept on revert to Claude** (2026-05-04) as belt-and-suspenders: harmless on Claude (which rarely emits `<internal>` in tool args), and the defensive "refuse if empty after strip" behavior is good regardless of provider. Drop into Milton if Claude ever starts doing the same thing.
